...
import { AddMovieContextForm } from './AddMovieContextForm'
import { Badge } from '@/components/ui/badge'
import { Button } from '@/components/ui/button'
import Link from 'next/link'
import { Film } from 'lucide-react'

export default async function MovieProfilePage() {
  ...
  if (!profile) {
    return (
      <div className="text-center py-24 text-muted-foreground max-w-sm mx-auto">
        ...
        <p className="text-sm mb-4">Rate a few movies and your profile will be generated automatically.</p>
        <Button asChild><Link href="/movies">Go to movies</Link></Button>
      </div>
    )
  }

  const genres = parseJson<string[]>(profile.favoriteGenres, [])
  const directors = parseJson<string[]>(profile.favoriteDirectors, [])
  ...
  return (
    <div className="max-w-xl space-y-8">
      ...
      <div className="grid grid-cols-2 gap-6">
        ...
        {directors.length > 0 && (
          <div>
            <h3 className="text-xs font-medium uppercase tracking-widest text-muted-foreground mb-2">Directors</h3>
            <div className="flex flex-wrap gap-1.5">
              {directors.map(d => <Badge key={d} variant="outline">{d}</Badge>)}
            </div>
          </div>
        )}
        {moods.length > 0 && (
