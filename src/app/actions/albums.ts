...
export async function generateRecommendationsAction(prompt: string, useTasteProfile = true) {
  ...
  const artistPicks = await callClaudeJson<{ artist: string; reason: string }[]>(p1, { model: OPUS, maxTokens: 2048 })
  ...
  const suggestions = await callClaudeJson<{ artist: string; title: string; year?: number; genre?: string }[]>(p2, { model: OPUS, maxTokens: 2048 })
