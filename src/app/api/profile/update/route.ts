...
export async function POST(req: Request) {
  ...
  if (!triggerEntry) return new Response('Not found', { status: 404 })

  // Run profile update in background (don't block the response)
  updateProfile(entryId).catch(console.error)

  return new Response('OK')
}

async function updateProfile(entryId: number) {
  ...
  const entriesText = recentEntries
    .map(entry => {
      const chat = entry.messages
        ...
        .join('\n')
      const d = new Date(entry.date + 'T12:00:00').toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })
  ...
  const profilePrompt =
    ...
    '}\n\n' +
    'Be specific and grounded in actual evidence from their writing. Avoid generic observations. Return ONLY the JSON object, no other text.'
  ...
  // Update entry meta (mood, summary, themes)
  const triggerEntry = await db.journalEntry.findUnique({ where: { id: entryId } })
